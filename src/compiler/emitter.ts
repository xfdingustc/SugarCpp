...
namespace sc {
    ...
    export function emitFiles(resolver: EmitResolver, host: EmitHost, targetSourceFile: SourceFile, emitOnlyDtsFiles?: boolean, transformers?: TransformerFactory<SourceFile>[]): EmitResult {
        ...
        const headerWriter = createTextWriter(newLine);
        ...
        function emitSourceFileOrBundle({ hppFilePath, cppFilePath, sourceMapFilePath, declarationFilePath }: EmitFileNames, sourceFileOrBundle: SourceFile | Bundle) {
            ...
            if (!host.isEmitBlocked(hppFilePath) && !host.isEmitBlocked(cppFilePath) && !compilerOptions.noEmit) {
                printSourceFileOrBundle(hppFilePath, cppFilePath, sourceMapFilePath, sourceFileOrBundle);
            }
            else {
                emitSkipped = true;
            }
        ...
        function printSourceFileOrBundle(hppFilePath: string, cppFilePath: string, sourceMapFilePath: string, sourceFileOrBundle: SourceFile | Bundle) {
            ...
            if (bundle) {
                ...
                isOwnFileEmit = true;
                printer.writeFile(sourceFile, headerWriter, writer);
                sys.write("writer: " + writer.getText() + sys.newLine)
            }
            ...
            writeFile(host, emitterDiagnostics, cppFilePath, writer.getText(), compilerOptions.emitBOM, sourceFiles);
    ...
    export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
        ...
        let headerWriter: EmitTextWriter;
        ...
        function printFile(sourceFile: SourceFile): string {
            writeFile(sourceFile, beginPrint(), beginPrint());
            return endPrint();
        }
        ...
        function writeFile(sourceFile: SourceFile, headerOutput: EmitTextWriter, output: EmitTextWriter) {
            const previousWriter = writer;
            setHeaderWriter(headerOutput);
            setWriter(output);
            print(EmitHint.SourceFile, sourceFile, sourceFile);
            reset();
            writer = previousWriter;
        ...
        function setHeaderWriter(output: EmitTextWriter | undefined) {
            headerWriter = output;
